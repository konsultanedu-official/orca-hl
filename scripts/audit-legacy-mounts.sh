...
docker inspect "$target" >/dev/null 2>&1 || fail "container not found: $target"

running="$(docker inspect -f '{{.State.Running}}' "$target")"
user="$(docker inspect -f '{{if .Config.User}}{{.Config.User}}{{else}}<image-default>{{end}}' "$target")"
...
echo
echo '=== Mounts ==='
docker inspect -f '{{range .Mounts}}{{printf "%s\t%s\t%s\tRW=%v\n" .Type .Source .Destination .RW}}{{end}}' "$target"

echo
echo '=== Legacy /root mount destinations ==='
legacy_mounts="$(docker inspect -f '{{range .Mounts}}{{println .Destination}}{{end}}' "$target" | awk '/^\/root(\/|$)/')"
